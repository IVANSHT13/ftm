import{a as e,i as t,n,r,t as i}from"./index-DZoVOHG7.js";var a=`<section class="page-card p-4 p-lg-5">\r
...
`,o=`Pro League | Login`;function s(){return a}function c(e,t,n=`info`){e.className=`alert alert-${n} mb-4`,e.textContent=t,e.classList.remove(`d-none`)}function l(e){e.className=`alert mb-4 d-none`,e.textContent=``}function u(e,t){e.forEach(e=>{e.disabled=t})}async function d(e,n,r){e.preventDefault(),l(n);let a=e.currentTarget,o=new FormData(a),s=String(o.get(`email`)||``).trim(),d=String(o.get(`password`)||``);if(!s||!d){c(n,`Enter your email and password to sign in.`,`warning`);return}u([r],!0);try{await t(s,d),i(`/dashboard`)}catch(e){c(n,e.message||`Unable to sign in right now.`,`danger`)}finally{u([r],!1)}}async function f(t,n,r){t.preventDefault(),l(n);let a=t.currentTarget,o=new FormData(a),s=String(o.get(`fullName`)||``).trim(),d=String(o.get(`registerEmail`)||``).trim(),f=String(o.get(`registerPassword`)||``),p=String(o.get(`role`)||`player`);if(!s||!d||!f){c(n,`Fill in your name, email, and password to create an account.`,`warning`);return}u([r],!0);try{if(await e({fullName:s,email:d,password:f,role:p})){i(`/dashboard`);return}c(n,`Account created. Confirm your email if required, then sign in.`,`success`),a.reset()}catch(e){c(n,e.message||`Unable to create the account right now.`,`danger`)}finally{u([r],!1)}}async function p(e){if(!r()){let t=e.querySelector(`#authStatus`);t&&c(t,`Supabase is not configured yet. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to enable login.`,`warning`);return}if(await n()){i(`/dashboard`);return}let t=e.querySelector(`#authStatus`),a=e.querySelector(`#loginForm`),o=e.querySelector(`#registerForm`),s=e.querySelector(`#loginButton`),l=e.querySelector(`#registerButton`);a&&t&&s&&a.addEventListener(`submit`,e=>{d(e,t,s)}),o&&t&&l&&o.addEventListener(`submit`,e=>{f(e,t,l)})}export{p as mount,s as render,o as title};