...
`,s=`Pro League | Login`;function c(){return o}function l(e,t,n=`info`){e.className=`alert alert-${n} mb-4`,e.textContent=t,e.classList.remove(`d-none`)}function u(e){e.className=`alert mb-4 d-none`,e.textContent=``}function d(e,t){e.forEach(e=>{e.disabled=t})}function f(e){return e===`admin`?`/admin`:`/dashboard`}async function p(e){return f((await r(e.user.id).catch(()=>null))?.role||e.user.user_metadata?.role||`player`)}async function m(e,t,r){e.preventDefault(),u(t);let i=e.currentTarget,o=new FormData(i),s=String(o.get(`email`)||``).trim(),c=String(o.get(`password`)||``);if(!s||!c){l(t,`Enter your email and password to sign in.`,`warning`);return}d([r],!0);try{let e=await n(s,c);a(e?await p(e):`/dashboard`)}catch(e){l(t,e.message||`Unable to sign in right now.`,`danger`)}finally{d([r],!1)}}async function h(e,t,n){e.preventDefault(),u(t);let r=e.currentTarget,o=new FormData(r),s=String(o.get(`fullName`)||``).trim(),c=String(o.get(`registerEmail`)||``).trim(),p=String(o.get(`registerPassword`)||``),m=String(o.get(`role`)||`player`);if(!s||!c||!p){l(t,`Fill in your name, email, and password to create an account.`,`warning`);return}d([n],!0);try{if(await i({fullName:s,email:c,password:p,role:m})){a(f(m));return}l(t,`Account created. Confirm your email if required, then sign in.`,`success`),r.reset()}catch(e){l(t,e.message||`Unable to create the account right now.`,`danger`)}finally{d([n],!1)}}async function g(n){if(!e()){let e=n.querySelector(`#authStatus`);e&&l(e,`Supabase is not configured yet. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to enable login.`,`warning`);return}let r=await t();if(r){a(await p(r));return}let i=n.querySelector(`#authStatus`),o=n.querySelector(`#loginForm`),s=n.querySelector(`#registerForm`),c=n.querySelector(`#loginButton`),u=n.querySelector(`#registerButton`);o&&i&&c&&o.addEventListener(`submit`,e=>{m(e,i,c)}),s&&i&&u&&s.addEventListener(`submit`,e=>{h(e,i,u)})}export{g as mount,c as render,s as title};